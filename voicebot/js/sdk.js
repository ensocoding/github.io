...
function WebLiveChatListener() {
    ...
    this.createBox = function() {
        ...
        var html = "<link rel='stylesheet' href='" + this.css + "' type='text/css'>\n";
    ...
    this.popup = function() {
        ...
        if (this.popupURL != null) {
            ...
            var form = this.baseRoot.createElement("form");
            form.setAttribute("method", "post");
            form.setAttribute("action", SDK.url + "/livechat");
            form.setAttribute("target", 'child');

            var input = this.baseRoot.createElement('input');
            input.type = "hidden";
            input.name = "id";
            input.value = this.instance;
            form.appendChild(input);

            input = this.baseRoot.createElement('input');
            input.type = "hidden";
            input.name = "embedded";
...
function WebChatbotListener() {
    ...
    this.createBox = function() {
        ...
        if (this.baseRoot.getElementById(this.prefix + "chooselanguage") != null) {
            this.baseRoot.getElementById(this.prefix + "chooselanguage").addEventListener("change", function() {
                ...
                if (self.lang != "none") {
                    self.baseRoot.getElementById(self.prefix + 'yandex').style.display = "inline";
                    self.nativeVoice = true;
                    self.translate = true;
                    self.voice = null;
                } else {
                    self.baseRoot.getElementById(self.prefix + 'yandex').style.display = "none";
    ...
    this.createLiveChatBox = function(channel, label, position) {
        var box = this.baseRoot.createElement('div');
        var buttonstyle = "";
        if (this.color != null) {
            buttonstyle = "background-color:" + this.color + ";";
        }
    ...
    this.initWebPreview = function() {
        var browser = this.baseRoot.createElement('div');
    ...
    this.POST = function(url, xml, processor) {
        ...
        request.onreadystatechange = function() {
            if (debug) {
                ...
                console.log(request.responseXML);
                // VICKY HACKING 2
                vicky_sdk_api_tracing(request.responseText);
            }
            if (request.readyState != 4) return;
            if (request.status != 200 && request.status != 204) {
                console.log('Error: SDK POST web request failed');
                // VICKY HACKING 2
                vicky_sdk_error_1('Error: SDK POST web request failed');
